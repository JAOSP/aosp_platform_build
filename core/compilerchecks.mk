...
# friends in the same file that sets TARGET_CC (and therefore we have to
# be included first)

ifeq ($(strip $(TARGET_TOOLS_PREFIX)),)
ifeq ($(TARGET_ARCH),arm)
COMPILERCHECK_CC := prebuilts/gcc/$(HOST_PREBUILT_TAG)/$(TARGET_ARCH)/arm-linux-androideabi-4.8/bin/arm-linux-androideabi-gcc$(HOST_EXECUTABLE_SUFFIX)
else
ifeq ($(TARGET_ARCH),aarch64)
COMPILERCHECK_CC := prebuilts/gcc/$(HOST_PREBUILT_TAG)/$(TARGET_ARCH)/aarch64-linux-android-4.8/bin/aarch64-linux-android-gcc$(HOST_EXECUTABLE_SUFFIX)
else
ifeq ($(TARGET_ARCH),x86)
COMPILERCHECK_CC := prebuilts/gcc/$(HOST_PREBUILT_TAG)/$(TARGET_ARCH)/x86_64-linux-android-4.8/bin/x86_64-linux-android-gcc$(HOST_EXECUTABLE_SUFFIX)
else
ifeq ($(TARGET_ARCH),mips)
COMPILERCHECK_CC := prebuilts/gcc/$(HOST_PREBUILT_TAG)/$(TARGET_ARCH)/mipsel-linux-android-4.8/bin/mipsel-linux-android-gcc$(HOST_EXECUTABLE_SUFFIX)
endif
endif
endif
endif
else
COMPILERCHECK_CC := $(TARGET_TOOLS_PREFIX)gcc$(HOST_EXECUTABLE_SUFFIX)
endif
...
cc-version = $(shell echo '__GNUC__ __GNUC_MINOR__' \
	|$(COMPILERCHECK_CC) -E -xc - |tail -n1 |sed -e 's, ,,g')

cc-ifversion = $(shell [ $(call cc-version) $(1) $(2) ] && echo $(3))

cc-option = $(call try-run, echo -e "$(1)" \
	|$(COMPILERCHECK_CC) $(1) -c -xc /dev/null -o /dev/null,$(1),$(2))
