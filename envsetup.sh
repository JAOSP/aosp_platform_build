...
function setpaths()
{
    ...
    export ANDROID_EABI_TOOLCHAIN2=
    ...
    if [ -d "$gccprebuiltdir/$toolchaindir2" ]; then
        export ANDROID_EABI_TOOLCHAIN2=$gccprebuiltdir/$toolchaindir2
    fi
    ...
    export ANDROID_TOOLCHAIN2=$ANDROID_EABI_TOOLCHAIN2
...
# determine whether it is a 32-bit or 64-bit executable.
function is64bit()
{
    local PID="$1"
    if [ "$PID" ] ; then
...
function gdbclient()
{
   ...
   if [ "$OUT_ROOT" -a "$PREBUILTS" ]; then
       ...
       # 64-bit exe found
       if [ "$USE64BIT" != "" ] ; then
           gdbwrapper $ANDROID_TOOLCHAIN/$GDB64 "$OUT_ROOT/gdbclient.cmds" "$OUT_EXE_SYMBOLS/$EXE"
       # 32-bit exe / 32-bit platform
       elif [ "$(get_build_var TARGET_2ND_ARCH)" = "" ]; then
           gdbwrapper $ANDROID_TOOLCHAIN/$GDB "$OUT_ROOT/gdbclient.cmds" "$OUT_EXE_SYMBOLS/$EXE"
       # 32-bit exe / 64-bit platform
       else
           gdbwrapper $ANDROID_TOOLCHAIN2/$GDB "$OUT_ROOT/gdbclient.cmds" "$OUT_EXE_SYMBOLS/$EXE"
       fi
  else
